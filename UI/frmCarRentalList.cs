...
{
    public partial class frmCarRentalList : Form
    {
        Connection.NobelMoonEntities1 entity = new Connection.NobelMoonEntities1();
        public frmCarRentalList()
        {
            InitializeComponent();
        }

        private void frmCarRentalList_Paint(object sender, PaintEventArgs e)
        {
        ...
        private void frmCarRentalList_Load(object sender, EventArgs e)
        {
        ...
        private void LoadPassportBookingList()
        {
            ...
            List<NobelMoon.Connection.GetCarRentalHeaderReport_Result> tbList = entity.GetCarRentalHeaderReport().Where(x => Convert.ToDateTime(x.InvoiceDate.Value.ToShortDateString()) >= Convert.ToDateTime(dtpFrom.Value.ToShortDateString()) && Convert.ToDateTime(x.InvoiceDate.Value.ToShortDateString()) <= Convert.ToDateTime(dtpTo.Value.ToShortDateString())).ToList();
            for (int i = 0; i < tbList.Count; i++)
            {
                dgvCarRental.Rows.Add(i + 1, tbList[i].InvoiceDate.Value.ToShortDateString(), tbList[i].InvoiceNo, tbList[i].ContactPerson, tbList[i].CompanyName, tbList[i].PaymentTerm, tbList[i].Currency, tbList[i].Rate, tbList[i].Balance, tbList[i].Status);
            }
        }
        ...
        private void dgvCarRental_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                string invoice = dgvCarRental.CurrentRow.Cells["colInvoice"].Value.ToString();
                int count = (Int32)entity.SP_ChkTransactionForSaleEditDelete(invoice, "CarRental").FirstOrDefault();
                if (count > 0)
                {
                ...
                else
                {
                    if (e.ColumnIndex == 10)
                    {
                        frmCarRental newform = new frmCarRental();
                        newform.InvoiceNo = invoice;
                        newform.IsEdit = true;
                        newform.btnCBooking.Text = "Update";
                        this.Hide();
                        newform.ShowDialog();
                        this.Show();
                    }
